...
<div id="page-container">
	<nav class="admin-panel">
		...
		{:then tournamentList}
			...
			<button on:click={generate_new_tournament}> + </button>
		{:catch error}
			{error}
		{/await}
	</nav>
		...
		{:else}
			<form on:submit|preventDefault={submit_tournament}>
				...
				<label style="display: grid; grid-template-columns: auto 1fr; grid-gap: 1rem;">
					...
					<input
						type="text"
						bind:value={current_tournament.tournament_name}
						maxlength="40"
						on:change={() => (form_changed = true)}
						required
					/>
				</label>
				...
				<label style="display: grid; grid-template-columns: auto 1fr; grid-gap: 1rem;">
					...
					<input
						type="text"
						bind:value={current_tournament.tournament_sponsor}
						maxlength="40"
						on:change={() => (form_changed = true)}
						required
					/>
				...
				<img
					on:click={() => {
						document.getElementById('tournament-image-picker').click();
					}}
					class="tournament-image"
					alt="turnering billede"
					src={current_tournament.tournament_image
						? current_tournament.tournament_image
						: '/default-header/medium.avif'}
				/>
				<input
					style="display:none"
					id="tournament-image-picker"
					type="file"
					accept="image/*"
					on:change={(event) => change_image(event)}
				/>
					...
					<details>
						...
						<div class="hole-card">
							<img
								...
								src={hole.hole_image ? hole.hole_image : '/default-header/medium.avif'}
							/>
							<input
								style="display:none"
								id={i}
							...
							<div class="hole-form">
								...
								{#each hole.scores as score, i}
									<div
										style="display: grid; grid-template-columns: 1fr 1fr auto; grid-gap: 0.5rem; width: 100%;"
									>
			...
			<button on:click={delete_tournament}> Slet turnering </button>

			<button on:click={duplicate_tournament}> Dupliker turnering </button>
		{/if}
	</main>
</div>
